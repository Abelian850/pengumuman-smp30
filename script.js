...
function tampilkanPengumuman(siswa, isFound) {
  ...
  $('dokAvatar').textContent   = getInitials(siswa.nama);
  $('dokNama').textContent     = toTitleCase(siswa.nama) || '—';
  $('dokSub').textContent      = [
    siswa.nisn   ? `NISN: ${siswa.nisn}`       : null,
    siswa.kelas  ? `Kelas ${siswa.kelas}`       : null,
    siswa.angkatan ? `Angkatan ${siswa.angkatan}` : null,
  ].filter(Boolean).join(' · ');

  // Cek dokumen Drive — defensive: ada_dokumen bisa tidak ada jika Apps Script lama
  const adaDrive = siswa.ada_dokumen === true && siswa.link_dokumen;

  if (adaDrive) {
    // Tombol download
    $('dokBtnDownload').href          = siswa.link_dokumen;
