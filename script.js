...
function tampilkanPengumuman(siswa, isFound) {
  ...
  const adaDrive  = siswa.ada_dokumen && siswa.link_dokumen;
  ...
  if (isLulus) {
    ...
    // Info grid
    $('ttlLulus').textContent   = `${toTitleCase(siswa.tempat_lahir) || '—'}, ${formatTanggal(siswa.tanggal_lahir)}`;
    $('jkLulus').textContent    = formatJK(siswa.jk);
    $('kelasLulus').textContent = siswa.kelas   || '—';
    $('nisnLulus').textContent  = siswa.nisn    || '—';
    $('hasilLulus').style.display  = 'block';
    buatKonfeti();

    // Tombol Drive / placeholder LULUS
    const btnDrive = $('btnDriveLulus');
    const placeholder = $('placeholderLulus');
    const previewWrap = $('previewWrapLulus');
    const previewFrame = $('previewFrameLulus');
    const previewLoading = $('previewLoadingLulus');

    if (adaDrive) {
      btnDrive.href          = siswa.link_dokumen;
      btnDrive.style.display = 'flex';
      placeholder.style.display = 'none';

      // Tampilkan preview — konversi link download ke embed viewer
      const embedUrl = linkKeEmbed(siswa.link_dokumen);
      if (embedUrl) {
        previewWrap.style.display = 'block';
        previewFrame.src = embedUrl;
        // Sembunyikan loading spinner setelah iframe selesai load
        previewFrame.onload = () => {
          previewLoading.classList.add('hidden');
        };
        // Fallback: sembunyikan loading setelah 8 detik
        setTimeout(() => previewLoading.classList.add('hidden'), 8000);
      }
    } else {
    ...
    // Info grid
    $('ttlTidak').textContent   = `${toTitleCase(siswa.tempat_lahir) || '—'}, ${formatTanggal(siswa.tanggal_lahir)}`;
    $('jkTidak').textContent    = formatJK(siswa.jk);
    $('kelasTidak').textContent = siswa.kelas   || '—';
    $('nisnTidak').textContent  = siswa.nisn    || '—';
    $('hasilTidak').style.display  = 'block';

    // Tombol Drive / placeholder TIDAK LULUS
    const btnDrive = $('btnDriveTidak');
    const placeholder = $('placeholderTidak');
    const previewWrap = $('previewWrapTidak');
    const previewFrame = $('previewFrameTidak');
    const previewLoading = $('previewLoadingTidak');
...
/* Tombol "Lihat Surat" → tampilkan surat yang di-generate website */
