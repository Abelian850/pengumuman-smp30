...
/* Tampilkan halaman dokumen langsung setelah login berhasil */
function tampilkanPengumuman(siswaRaw, isFound) {
  sembunyikanSemua();
  $('pengumumanSection').style.display = 'flex';
  window.scrollTo({ top: 0, behavior: 'smooth' });

  // Reset semua elemen
  $('dokBtnDownload').style.display  = 'none';
  $('dokPlaceholder').style.display  = 'none';
  $('dokPreviewWrap').style.display  = 'none';
  $('dokError').style.display        = 'none';
  $('dokHeader').style.display       = 'none';

  // Login gagal / data tidak ditemukan
  if (!isFound || !siswaRaw) {
    $('dokError').style.display = 'flex';
    return;
  }

  // Normalisasi data — Apps Script bisa kirim berbagai format key
  // Coba baca dari object langsung, atau dari nested 'data'
  const s = siswaRaw.data || siswaRaw;

  // Helper baca field dengan fallback berbagai nama key
  const baca = (...keys) => {
    for (const k of keys) {
  ...
  const nama     = baca('nama','NAMA','name');
  const nisn     = baca('nisn','NISN');
  const kelas    = baca('kelas','KELAS','class');
  const angkatan = baca('angkatan','ANGKATAN') || '2026';
  const linkDok  = baca('link_dokumen','linkDokumen','link','url');
  const adaDok   = (s.ada_dokumen === true || s.ada_dokumen === 'true') && linkDok;

  // Isi identitas header
  $('dokHeader').style.display = 'flex';
  $('dokAvatar').textContent   = getInitials(nama) || '?';
  $('dokNama').textContent     = toTitleCase(nama) || '—';
  $('dokSub').textContent      = [
    nisn     ? `NISN: ${nisn}`          : null,
  ...
  if (adaDok) {
    // Tombol download
    $('dokBtnDownload').href          = linkDok;
    $('dokBtnDownload').style.display = 'flex';

    // Preview iframe
    const embedUrl = linkKeEmbed(linkDok);
    if (embedUrl) {
      $('dokPreviewWrap').style.display = 'block';
      const frame   = $('dokPreviewFrame');
      const loading = $('dokPreviewLoading');
    ...
    // Dokumen belum tersedia di Drive
    $('dokPlaceholder').style.display = 'flex';
  }
}

/* Tombol "Lihat Surat" — tidak dipakai lagi tapi dijaga agar tidak error */
...
async function doLogin() {
  ...
  try {
    ...
    if (data.status === 'found') {
      ...
      console.log('DATA SISWA dari API:', JSON.stringify(data.siswa, null, 2));
...
// =====================================================
// KONVERSI LINK DRIVE → EMBED VIEWER
// =====================================================
/**
 * Konversi berbagai format URL Google Drive ke URL embed preview.
 * Input:  https://drive.google.com/uc?export=download&id=FILE_ID
 *         https://drive.google.com/file/d/FILE_ID/view
 * Output: https://drive.google.com/file/d/FILE_ID/preview
 */
function linkKeEmbed(url) {
  if (!url) return null;
  // Coba ambil FILE_ID dari format /uc?export=download&id=FILE_ID
  const m1 = url.match(/[?&]id=([a-zA-Z0-9_-]+)/);
  if (m1) return `https://drive.google.com/file/d/${m1[1]}/preview`;
  // Coba ambil FILE_ID dari format /file/d/FILE_ID/
  ...
  window.fetch = async (url) => {
    ...
    const db = {
      '0098765432': {
        ...
        ada_dokumen: true,
        link_dokumen: 'https://drive.google.com/uc?export=download&id=CONTOH_FILE_ID',
      },
      '0011223344': {
        ...
        ada_dokumen: false,   // simulasi: dokumen belum upload
        link_dokumen: null,
      },
      '0055667788': {
        ...
        ada_dokumen: true,
        link_dokumen: 'https://drive.google.com/uc?export=download&id=CONTOH_FILE_ID_2',
      },
    };
